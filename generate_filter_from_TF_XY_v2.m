function generate_filter_from_TF_XY_v2(foldername, vec_len, num_f, den_f, filter_name,...
    filter_base_type, print_test_data, datasection, alignment, integrator_max, vec_len2, num_f2, den_f2)
% V2: Added support for different vec_len and vec_len2
%
...
% Allows for another filter based on a XDIR={0,1} switch.
%
...
den_f2 = den_f2(1 : find(den_f2, 1, 'last'));
num_f2 = num_f2(1 : find(num_f2, 1, 'last'));

% Check lengths
n_den = length(den_f);
n_num = length(num_f);
assert(length(den_f)==length(den_f2));
assert(length(num_f)==length(num_f2));
% assert(n_den >= n_num);
...
if (abs(den_f2(1)) > 0) && (den_f2(1) ~= 1)
    num_f2 = num_f2 ./ den_f2(1);
    den_f2(2:end) = den_f2(2:end) ./ den_f2(1);
    den_f2(1) = 1;
end
...
fprintf(fid_c, "#else\n");
for i = 2 : n_den
    fprintf(fid_c, tmplate_y, ftype, prefix, i-1, ftype, -den_f2(i));
end
for i = 1 : n_num
    fprintf(fid_c, tmplate_u, ftype, prefix, i-1, ftype, num_f2(i));
end
fprintf(fid_c, "#endif // XDIR\n");
fprintf(fid_c, "#ifdef SOC_C6678\n");
fprintf(fid_c, '#pragma SET_DATA_SECTION()\n');
fprintf(fid_c, "#endif // SOC_C6678\n");
fprintf(fid_c, '\n\n');
