...
% add oddities  (some correctors have been exchanged in the D-I ring)
aslowx = 2*pi*10;
afastx = 2*pi*2000;
...
bw = 2*pi*200;
abw = exp(-bw*Ts);
T_mp_z = (1-abw) / (1-z^(-1)*abw) * z^(-1);  % desired closed-loop
...
%% Reference signal
len_ref = n_samples;
t_ref = (0 : 1 : n_samples-1)*Ts;
ref = zeros(size(doff));
ibpm_ref = 1;
w_ref = 2*pi / 0.5;
A_ref = 5; % in micrometers
sig_ref = A_ref*sin(w_ref*t_ref);
ref(ibpm_ref,:) = sig_ref;
...
open_loop = false;
run_ntimes = 1;
[y_sim, u_sim] = sim_standard_imc_reference(...
                    n_samples, n_delay, id_to_bpm, id_to_cm, doff,...
                    Ac, Bc, Cc, Dc,... % CONTROLLER STATE-SPACE
                    A, B, C, D, network_scaling, ref,... % PLANT STATE-SPACE
                    open_loop, 1);
...
fig = figure;
subplot(1,3,1); 
plot(t,doff(ibpm_ref,:)'); title(sprintf('Disturbance %s',dirs{pick_dir}));
ylim([min(min(doff(ibpm_ref,:))), max(max(doff(ibpm_ref,:)))]);
xlabel('Time [s]'); ylabel('Position [mum]');

subplot(1,3,2);
plot(t,y_sim(:,ibpm_ref:ibpm_ref)); title(sprintf('Output %s',dirs{pick_dir}))
ylim([min(min(y_sim(:,ibpm_ref))), max(max(y_sim(:,ibpm_ref)))]);
xlabel('Time [s]'); ylabel('Position [mum]');

subplot(1,3,3); plot(t,u_sim(:,ibpm_ref)); title(sprintf('Setpoint %s',dirs{pick_dir}))
xlabel('Time [s]'); ylabel('Setpoint [A]');
