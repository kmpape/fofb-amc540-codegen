...
fnames_FA = {sprintf('../../DATA/28092022_002500_data_%s.mat',dirs{pick_dir}),... % FOFB OFF
             sprintf('../../DATA/28092022_002510_data_%s.mat',dirs{pick_dir}),... % FOFB OFF
             sprintf('../../DATA/28092022_002520_data_%s.mat',dirs{pick_dir}),... % FOFB OFF
             sprintf('../../DATA/28092022_002530_data_%s.mat',dirs{pick_dir}),... % FOFB OFF
             sprintf('../../DATA/28092022_002540_data_%s.mat',dirs{pick_dir})};
...
c_zx = eps_factor * q_zx / (1 - eps_factor * T_mp_z*z^(-n_delay));
c_zy = eps_factor * q_zy / (1 - eps_factor * T_mp_z*z^(-n_delay));
...
figure;
plot(t,u_sim); title(sprintf('Setpoint %s, BW=%.0f Hz, 1-eps=%e',dirs{pick_dir},bw/2/pi,eps_factor))
xlabel('Time [s]'); ylabel('Setpoint [A]');

dss = mean(doff(:,end-1000:end),2); 
u_sim_end = -pinv(RM)*doff(id_to_bpm,end);
u_sim_end = -pinv(RM)*dss(id_to_bpm);
figure;
plot(id_to_cm,u_sim(end,id_to_cm),'b', id_to_cm, u_sim_end/1000,'r');
title(sprintf('Setpoint %s, BW=%.0f Hz, 1-eps=%e',dirs{pick_dir},bw/2/pi,eps_factor))
xlabel('Corrector [-]'); ylabel('Setpoint [A]');

figure;
plot(not_ctr,yss(not_ctr),'--','color',[0.9 0.9 0.9]); hold on; 
plot(id_to_bpm,yss(id_to_bpm)); title(sprintf('Steady-State %s (10k samples), BW=%.0f Hz, 1-eps=%e',dirs{pick_dir},bw/2/pi,eps_factor))
ylim([min(min(yss(id_to_bpm))), max(max(yss(id_to_bpm)))]);
xlabel('BPM [-]'); xlim([1 TOT_BPM]); ylabel('Position [mum]');

doffavg = movmean(doff(id_to_bpm,:)',5000,1);
figure;
plot(t,doffavg); title(sprintf('Disturbance %s (active BPMs)',dirs{pick_dir}));
xlabel('Time [s]'); ylabel('Setpoint [mum]');

figure;
plot(1:size(doffavg,2),doffavg(end,:)); title(sprintf('Disturance Final Values %s',dirs{pick_dir}));
ylim([min(doff(id_to_bpm,end)), max(doff(id_to_bpm,end))]);
xlabel('BPM [-]'); xlim([1 TOT_BPM]); ylabel('Position [mum]');

figure;
plot(t,doff(116,:)); title(sprintf('Disturbance %s',dirs{pick_dir})); hold on;
plot(t,detrend(doff(116,:)));
xlabel('Time [s]'); ylabel('Position [mum]');
...
subplot(4,1,1); 
%if ~isempty(not_ctr); plot(t,doff(not_ctr,:)','--','color',[0.9 0.9 0.9]); hold on; end
plot(t,doff(24,:)'); title(sprintf('Disturbance %s',dirs{pick_dir})); hold on;
plot(t,movmean(doff(24,:)',avgk),'r');
ylim([min(min(doff(24,:))), max(max(doff(24,:)))]); xlim([t(1) t(end)]);
xlabel('Time [s]'); ylabel('Position [mum]');

subplot(4,1,2);
%if ~isempty(not_ctr); plot(t,y_sim(:,not_ctr),'--','color',[0.9 0.9 0.9]); hold on; end
plot(t,y_sim(:,24)); title(sprintf('Output %s',dirs{pick_dir})); hold on;
plot(t,movmean(y_sim(:,24)',avgk),'r');
ylim([min(min(y_sim(:,24))), max(max(y_sim(:,24)))]); xlim([t(1) t(end)]);
xlabel('Time [s]'); ylabel('Position [mum]');

subplot(4,1,3); plot(not_ctr,yss(not_ctr),'--','color',[0.9 0.9 0.9]); hold on; 
plot(id_to_bpm,yss(id_to_bpm)); title(sprintf('Steady-State %s',dirs{pick_dir}))
ylim([min(min(yss(id_to_bpm))), max(max(yss(id_to_bpm)))]);
xlabel('BPM [-]'); xlim([1 TOT_BPM]); ylabel('Position [mum]');

subplot(4,1,4); plot(t,u_sim(:,24)); title(sprintf('Setpoint %s',dirs{pick_dir}))
xlabel('Time [s]'); ylabel('Setpoint [A]'); xlim([t(1) t(end)]);
