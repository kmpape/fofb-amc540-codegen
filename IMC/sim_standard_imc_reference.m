function [y_sim, u_sim] = sim_standard_imc_reference(...
    nsamples, n_delay, id_to_bpm, id_to_cm, dist,...
    Ac, Bc, Cc, Dc,... % CONTROLLER STATE-SPACE
    A, B, C, D, network_scaling, ref, open_loop, run_ntimes) % PLANT STATE-SPACE
...
assert(size(dist, 1) == size(ref, 1))
assert(size(dist, 2) == size(ref, 2))
...
for mm = 1 : run_ntimes
    for ii = 1 : nsamples
        ...
        if (ii > n_delay) || (mm > 1)
            ...
            if use_single == true
                y_meas = single(round((y_sim(id_to_bpm,ind_ii)-ref(id_to_bpm,ii))*1000,0));
            else
                y_meas = double(round((y_sim(id_to_bpm,ind_ii)-ref(id_to_bpm,ii))*1000,0));
            end
