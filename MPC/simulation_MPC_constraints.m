...
[y_sim,u_sim,~,obs_y,obs_u,obs_x0,obs_xd,fgm_x0,fgm_xd,fgm_u,fgm_out,lower_u,upper_u] = sim_mpc(...
                ...
                SOFB_setp,false);
% [y_sim,u_sim,obs_y,obs_u,obs_x0,obs_xd,fgm_x0,fgm_xd,fgm_u,fgm_out,lower_u,upper_u] = sim_mpc_modespace(...
%                 n_samples, n_delay, doff,...
%                 Ap, Bp, Cp, ... % Plant
%                 Ao, Bo, Co, Ad, Cd, Lx8_obs, Lxd_obs,... % Observer
%                 J_mpc , q_mat, beta_fgm, RM,... % FGM
%                 u_max , u_rate,... % FGM
%                 id_to_bpm, id_to_cm,...
%                 ss_awr.A,ss_awr.B,ss_awr.C,ss_awr.D,...
%                 SOFB_setp,false);
...
u_sim_mode = u_sim(:,id_to_cm)*VR;
ysim_lqr_mode = y_sim_lqr(:,id_to_bpm)*UR;
u_sim_lqr_mode = u_sim_lqr(:,id_to_cm)*VR;
fig = figure;
...
%%
new_max_rate = u_rate;
new_max_rate(36) = 1*u_rate(36);

[y_sim_lqr_noctr, u_sim_lqr_noctr, x_sim_lqr_noctr] = sim_lqr(...
                n_samples, n_delay, doff,...
                Ap, Bp, Cp,... % plant
                Aobs, Bobs, Cobs, Kf, Kc,... % observer and regulator
                id_to_bpm, id_to_cm,...
...
[y_sim, u_sim, x_sim,~,~,~,~,~,~,~,~,~,~] = sim_mpc(...
                n_samples, n_delay, doff,...
                Ap, Bp, Cp, ... % Plant
                Ao, Bo, Co, Ad, Cd, Lx8_obs, Lxd_obs,... % Observer
                J_mpc , q_mat, beta_fgm,... % FGM
                u_max , new_max_rate,... % FGM
                id_to_bpm, id_to_cm,...
                ss_awr.A,ss_awr.B,ss_awr.C,ss_awr.D,...
                SOFB_setp,false);

[y_sim_lqr, u_sim_lqr, x_sim_lqr] = sim_lqr_w_constraints(...
                n_samples, n_delay, doff,...
                Ap, Bp, Cp,... % plant
                Aobs, Bobs, Cobs, Kf, Kc,... % observer and regulator
                id_to_bpm, id_to_cm,...
...
figure; 
plot(1:1:172,u_sim_lqr_noctr(303,:)-u_sim_lqr(303,:),'linestyle','none','marker','x'); hold on; 
plot(1:1:172,u_sim_lqr_noctr(303,:)-u_sim(303,:),'linestyle','none','marker','o');
legend("LQR vs LQR constraints", "LQR vs MPC")
...
%
costs = zeros(3,size(u_sim,1));

for k=1:size(u_sim,1)
    costs(1,k) = x_sim_lqr_noctr(k,id_to_cm)*Q_mpc*x_sim_lqr_noctr(k,id_to_cm)'+...
        u_sim_lqr_noctr(k,id_to_cm)*R_mpc*u_sim_lqr_noctr(k,id_to_cm)';
    costs(2,k) = x_sim_lqr(k,id_to_cm)*Q_mpc*x_sim_lqr(k,id_to_cm)'+...
        u_sim_lqr(k,id_to_cm)*R_mpc*u_sim_lqr(k,id_to_cm)';
    costs(3,k) = x_sim(k,id_to_cm)*Q_mpc*x_sim(k,id_to_cm)'+...
        u_sim(k,id_to_cm)*R_mpc*u_sim(k,id_to_cm)';
end


figure;
plot(1:1:6000, costs(1,:)); hold on;
plot(1:1:6000, costs(2,:));
plot(1:1:6000, costs(3,:),'--');
legend({"LQR no constraints","LQR w. enforced constraints","MPC"});


figure;
plot(1:1:400, costs(1,1:400)); hold on;
plot(1:1:400, costs(2,1:400));
plot(1:1:400, costs(3,1:400),'--');
legend({"LQR no constraints","LQR w. enforced constraints","MPC"});


figure;
plot(1:1:6000, cumsum(costs(1,:))); hold on;
plot(1:1:6000, cumsum(costs(2,:)));
plot(1:1:6000, cumsum(costs(3,:)),'--');
legend({"LQR no constraints","LQR w. enforced constraints","MPC"});
...
figure;
plot(300:1:1000, cumsum(costs(1,300:1:1000))); hold on;
plot(300:1:1000, cumsum(costs(2,300:1:1000)));
plot(300:1:1000, cumsum(costs(3,300:1:1000)),'--');
legend({"LQR no constraints","LQR w. enforced constraints","MPC"});
