function [y_sim,u_sim,...
    ...
    fgm_x0,fgm_xd,fgm_u,fgm_out,lower_u,upper_u] = sim_mpc_modespace(...
            n_samples, n_delay, dist,...
            Ap, Bp, Cp,... % Plant
            Ao, Bo, Co, Ad, Cd, LxN_obs, Lxd_obs,... % Observer
            J_MPC, q_mat, beta_fgm, RMobs,... % FGM
            u_max, u_rate,...
            id_to_bpm, id_to_cm,...
...
for k = 1:1:n_samples
    ...
    if k > n_delay
        ...
        if use_single == true
            out_global = single(VV'*u_sim(id_to_cm,k-1));
        else            
            out_global = double(VV'*u_sim(id_to_cm,k-1));
        end
        ...
        fgm_result = VV*z_new; % new: use z_new instead of out_global
