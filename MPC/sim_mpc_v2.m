function [y_sim,u_sim,...
    ...
    fgm_x0,fgm_xd,fgm_u,fgm_out] = sim_mpc_v2(...
...
% this version is depreciated. use sim_mpc instead
...
if use_single == true
    ...
    x8_obs_old=single(zeros(nx_obs,1)); x9_obs_old=single(zeros(nx_obs,1));
    xd_obs_old=single(zeros(ny_obs,1));
    
    x0_obs_new=single(zeros(nx_obs,1)); x1_obs_new=single(zeros(nx_obs,1));
    x2_obs_new=single(zeros(nx_obs,1)); x3_obs_new=single(zeros(nx_obs,1));
    x4_obs_new=single(zeros(nx_obs,1)); x5_obs_new=single(zeros(nx_obs,1));
    x6_obs_new=single(zeros(nx_obs,1)); x7_obs_new=single(zeros(nx_obs,1));
    x8_obs_new=single(zeros(nx_obs,1)); x9_obs_new=single(zeros(nx_obs,1));
    xd_obs_new=single(zeros(ny_obs,1));
    ...
    x8_obs_old=double(zeros(nx_obs,1)); x9_obs_old=double(zeros(nx_obs,1));
    xd_obs_old=double(zeros(ny_obs,1));
    
    x0_obs_new=double(zeros(nx_obs,1)); x1_obs_new=double(zeros(nx_obs,1));
    x2_obs_new=double(zeros(nx_obs,1)); x3_obs_new=double(zeros(nx_obs,1));
    x4_obs_new=double(zeros(nx_obs,1)); x5_obs_new=double(zeros(nx_obs,1));
    x6_obs_new=double(zeros(nx_obs,1)); x7_obs_new=double(zeros(nx_obs,1));
    x8_obs_new=double(zeros(nx_obs,1)); x9_obs_new=double(zeros(nx_obs,1));
    xd_obs_new=double(zeros(ny_obs,1));
...
for k = 1:1:n_samples
    ...
    if k > n_delay
        ...
        if (k < 30)
            norm(z_new-z_old)/norm(z_old)*100.0
        end
        ...
        xd_obs_new = Ad*xd_obs_new;
        if (n_delay==9); x9_obs_new = x8_obs_new; end
        x8_obs_new = x7_obs_new;
        x7_obs_new = x6_obs_new;
        x6_obs_new = x5_obs_new;
        x5_obs_new = x4_obs_new;
        x4_obs_new = x3_obs_new;
        x3_obs_new = x2_obs_new;
        x2_obs_new = x1_obs_new;
        x1_obs_new = x0_obs_new;
        if use_single == true
            x0_obs_new = Ao*x0_obs_new + Bo*single(u_sim(id_to_cm,k));
        else
            x0_obs_new = Ao*x0_obs_new + Bo*double(u_sim(id_to_cm,k));
        end
