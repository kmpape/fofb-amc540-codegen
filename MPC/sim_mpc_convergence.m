function [y_sim,u_sim,x_sim,...
    n_iters,abs_err_qp,rel_err_qp] = sim_mpc_convergence(...
...
n_iters = zeros(1,n_samples);
abs_err_qp = zeros(1,n_samples);
rel_err_qp = zeros(1,n_samples);
MAX_ITER = 20;
for k = 1:1:n_samples
    ...
    if k > n_delay
        ...
        q_qp = qq_mat*double([x0_obs_new; xd_obs_new]); % for quadprog
        sol_qp = quadprog(JJ,q_qp,[],[],[],[],double(lower_u),double(upper_u),double(u_sim(id_to_cm,k-1)),optimset('Display', 'off'));
        for i_iter = 1 : 1 : MAX_ITER
            ...
            abs_err = norm(z_new-z_old,'Inf');
            if (abs_err < EPS_ABS) || (abs_err < EPS_REL * norm(z_old,'Inf'))
                break;
            end
        end
        n_iters(k) = i_iter;
        fgm_result = z_new; % new: use z_new instead of out_global
        abs_err_qp(k) = norm(double(z_new)-sol_qp, 'Inf');
        rel_err_qp(k) = abs_err_qp(k)/norm(sol_qp, 'Inf');
        ...
        if ~ol_mode
            ...
            u_sim(id_to_cm,k) = 0*double(fgm_result);
        end
