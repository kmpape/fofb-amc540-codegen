...
%% Look at data
inds = ind_start_u-10:ind_start_u+10;
figure;
plot(inds, data.u(1,inds),inds, data.y(1,inds)/10000)
ind_start_y = ind_start_u-1-n_delay;
% First nonzero input is at ind_start_u, but it is put one timestep later into the
% archive than it is applied. So it in fact starts at ind_start_u-1. This means that
% y(:,4459) is used to produce  the first input
...
%% Measurement Data
n_samples = 200;
inds = ind_start_y:ind_start_y+n_samples-1;
doff = data.y(:,inds);
u_FOFB = data.u(:,inds+1);
...
if true == true
    ...
    figure;
    subplot(3,2,1); plot(doff(id_to_bpm,:)'); title('Disturbance MPC');
    subplot(3,2,2); plot(y_sim(:,id_to_bpm)); title('Output MPC');
    subplot(3,2,3); plot(u_sim*scale_u); title('Input MPC');; ylim(ylim_);
    subplot(3,2,4); plot(u_sim(1:length(t),id_to_cm)*scale_u-y_awr); title('AWR MPC');
    subplot(3,2,5); plot(u_FOFB(id_to_cm,:)'); title('U FOFB'); ylim(ylim_);
end
asdf
