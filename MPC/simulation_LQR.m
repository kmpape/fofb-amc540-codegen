...
fname_mode = 'mode_by_mode_lqr_data.mat';
...
%%
inds_ss = 1000:n_samples;
[Sfiltered2, Sorig2, w_Hz] = estim_sensitivity(UR'*(y_sim_mode(inds_ss,id_to_bpm)'), UR'*doff(id_to_bpm,inds_ss), length(inds_ss), 1/Ts);

figure;

imodes = [20,30,40,50,60,70];
for i = 1:length(imodes)
    imode = imodes(i);
    subplot(2,length(imodes)/2,i);
    semilogx(w_Hz,20*log10(abs(Sorig2(:,imode))),'b'); hold on;
    semilogx(theo_data.w_Hz,Smin_theo,'k',theo_data.w_Hz,Smax_theo,'k'); hold on;
    semilogx(w_Hz, Sfiltered2(:,imode),'r');
    grid on; title(sprintf('MODE %d',imode))
end
