function [Ao_x, Bo_x, Co_x, Ap_x, Bp_x, Cp_x, Ad_x, Cd_x,...
          ...
          observer_regulator_bad(RMorigx,RMorigy,id_to_bpm_x, id_to_cm_x, id_to_bpm_y,id_to_cm_y,nd,fname,print_msg,Rsetting)
    if nargin < 8; fname = ''; end
    if nargin < 9; print_msg = false; end
    if nargin < 10; Rsetting = 'a'; end
    ...
    for i=1:2
        ...
        for imode = 1:ny
            ...
            if strcmp(Rsetting,'a')
                Rlqr = 1;
            elseif strcmp(Rsetting,'b')
                Rlqr = (mu+sval^2)/sval^2;
            elseif strcmp(Rsetting,'c')
                Rlqr = sqrt(sval);
            elseif strcmp(Rsetting,'d')
                Rlqr = sval;
            else
                assert(0);
            end
            ...
            OL = series(g,c_lqr);
            CL = 1/(1+OL);
            CL_w = freqresp(CL,w_Hz,'Hz'); CL_w = (CL_w(:));%CL_w = abs(CL_w(:));
            sensitivity_lqr(:,imode) = CL_w;
            tmp = freqresp(1/(1+g*c_imc),w_Hz,'Hz'); tmp = (tmp(:));%tmp = abs(tmp(:));
            sensitivity_imc(:,imode) = tmp;
            
            if (i==2) && ((imode==1)||(imode==50)||(imode==100))
                %figure; bode(OL); grid on;+
                figure;
                semilogx(w_Hz,20*log10(abs(CL_w)),'b'); hold on;
                semilogx(w_Hz,20*log10(abs(tmp)),'r');
                grid on;
                title(sprintf("SETTING = %s, MODE = %d", Rsetting,imode));
                
                cols = {'w','m','p'};
                filename = sprintf('thesis_csv/sens_%s_mode%d_LQG.csv',Rsetting,imode);
                mag = 20*log10(abs(CL_w));
                phase = 180/pi*angle(CL_w);
                data = [w_Hz', mag, phase];
                csvwrite_with_headers(filename, data, cols);
                filename = sprintf('thesis_csv/sens_%s_mode%d_IMC.csv',Rsetting,imode);
                mag = 20*log10(abs(tmp));
                phase = 180/pi*angle(tmp);
                data = [w_Hz', mag, phase];
                csvwrite_with_headers(filename, data, cols);
            end
        end
